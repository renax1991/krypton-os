...
void idt_handler(registers_t *regs) {
    if (interrupt_handlers [regs->int_no]) {
        ...
        if ((sys_base->sys_flags & NEED_SCHEDULE) != 0) {
        ...
        // Type in code to boom, bang and kill the offending thread
    }
}
...
void irq_handler(registers_t *regs) {
    ...
    outb(0x20, 0x20);
    // Call the interrupt request handler function
    if (interrupt_handlers[regs->int_no] != 0)
        interrupt_handlers[regs->int_no] (regs);

    // Check the "schedule needed" flag
    // If the flag is set, clear it call the scheduler
    if ((sys_base->sys_flags & NEED_SCHEDULE) != 0) {
        sys_base->sys_flags &= (~NEED_SCHEDULE);
        schedule();
    }
